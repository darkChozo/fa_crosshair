...
dc_fac_fnc_crosshairInit = {
	if (hasInterface && isNil {dc_fac_ev_crosshair}) then {
		...
		_briefingText = _briefingText + format ["
			<br/><execute expression=""profileNamespace setVariable ['dc_fac_var_crosshairEnable',true]"">Turn On Crosshair</execute>
			<br/><execute expression=""profileNamespace setVariable ['dc_fac_var_crosshairEnable',false]"">Turn Off Crosshair</execute><br/><br/>"];
		...
		dc_fac_ev_crosshair = addMissionEventHandler ["draw3D",{
			...
			{
				if (([_x,animationState player] call BIS_fnc_inString) && (profileNamespace getVariable "dc_fac_var_crosshairEnable")) exitWith {_showCrosshair = true};
			} forEach [
				"MstpSrasW",
				"MtacSrasW",
				"MwlkSrasW",
